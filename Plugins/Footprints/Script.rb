...
class Spriteset_Map
  ...
  def initialize(map=nil)
	...
	def viewport1 = @@viewport1
	
  def putFootprint(event, pos) = FootprintVariables.show(event, pos) 
...
class Game_Character
  ...
  def get_last_pos
	...
	def foot_prints? = $game_map.terrain_tag(get_last_pos[0], get_last_pos[1]) == FootprintVariables::TERRAIN_FOOT && $scene.is_a?(Scene_Map) && $scene.spriteset?
...
class Footsprite
  def initialize(sprite,event,viewport,map,direction,nid,chardata,player)
    ...
    @sprite  = Sprite.new(viewport)
		file = player && $PokemonGlobal.bicycle ? "footsetbike" : "footset"
		@sprite.bitmap = RPG::Cache.load_bitmap("Graphics/UI/Footprints/", file)
		# Set position
